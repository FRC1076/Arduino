...
void loop() {
  ...
  if (packetSize) {
    ...
    float brightness = root[String("brightness")];
    ...
    for (int i=0; i<num_pixels; i++) {
        strip.setPixelColor(root["pixel_values"][i], colorPalette[(int)root["pixel_colors"][i]]);
        Serial.println((int)root["pixel_colors"][1]);
    }
    //Serial.println("setting brighness to ");
    //Serial.println(trunc(baseBright * brightness));
    strip.show();
